export const computeFibonacciNumber = (position: number, recursion: boolean = false,  isRecursive: boolean = false): number => {
    ...
    if (isRecursive) return recursiveFibonacciNeg(position);
